...
//        private readonly int DataSetWidth = (int)DataSet.DataColumns.NumOfColumns;

//        private readonly int m_DataSetNumOfRows;

//        private readonly int m_AnalyzesNum = DataAnalyzer.AnalyzeCombinationItems.Count;

//        private readonly int PredictedCollectionsMaxSize;
...
//        public GPUAnalyzer(double[] dataset, int[] predictedCollections) : base(GPUModuleTarget.DefaultWorker)
//        {
//            m_DataSetNumOfRows = dataset.Length / DataSetWidth;
//            PredictedCollectionsMaxSize = predictedCollections.Length / m_AnalyzesNum;            
//        }
...
//        {
//            using (var loadedModule = new GPUAnalyzer(dataset, predictedCollections))
//            {
...
//                var lp = new LaunchParam(grid, block);
//                GPULaunch(AnalyzerKernel, lp, dDataset.Ptr, dCombinations.Ptr, dPredictedCollections.Ptr, dPredictedCollectionsSizes.Ptr,
//                    dAnalyzeCombinationsDataItems.Ptr, dAnalyzeCombinationsRanges.Ptr, dAnalyzeResults.Ptr);
//                return dAnalyzeResults.Gather();
//            }
//        }

//        [Kernel]
//        public void AnalyzerKernel(deviceptr<double> dataSet, 
//                                   deviceptr<ulong> combinations,
//                                   deviceptr<int> predictedCollections, 
//                                   deviceptr<int> predictedCollectionsSizes, 
//                                   deviceptr<int> analyzeCombinationsDataItems,
//                                   deviceptr<int> analyzeCombinationsRanges,
//                                   deviceptr<double> analyzeResults)
//        {
//            var combinationStart = blockIdx.x * blockDim.x + threadIdx.x;
//            var combinationStride = gridDim.x * blockDim.x;
...
//                {
//                    #region Do One Size Analyzer Combination           

//                    int combinationRowPtr = 0;
//                    int numOfCombinations = 0;
//                    int combinationRow = 0;
//                    var correctPredictions = __local__.Array<int>(m_AnalyzesNum);

//                    while (combinationRowPtr < predictedCollectionsSizes[combinationItem])
//                    {
//                        combinationRow = predictedCollections[combinationItem * PredictedCollectionsMaxSize + combinationRowPtr];
...
//                            #region Add If Contains Good Prediction
...
//                else
//                {
//                    //var combinationItems = __local__.Array<int>(DataAnalyzer.AnalyzeMaxCombinationSize);
//                    //var predictionPtrs = __local__.Array<int>(DataAnalyzer.AnalyzeMaxCombinationSize);
...
//                    //    if ((combination & ((ulong)1 << i)) != 0)
//                    //    {
//                    //        combinationItems[currentItem] = i;
//                    //    }
//                    //}

//                    //#region DoAnalyzerCombination
...
//                    //while (true)
//                    //{
//                    //    #region  Get Combination Row
//                    //    while (predictionPtrs[0] < predictedCollectionsSizes[combinationItems[0]])
//                    //    {
//                    //        for (int i = 0; i < combinationSize - 1; i++)
//                    //        {
//                    //            int j = i + 1;
//                    //            while (predictedCollections[combinationItems[i] * PredictedCollectionsMaxSize + predictionPtrs[i]] > predictedCollections[combinationItems[j] * PredictedCollectionsMaxSize + predictionPtrs[j]])
//                    //            {
//                    //                predictionPtrs[j]++;

//                    //                if (predictionPtrs[j] < predictedCollectionsSizes[combinationItems[j]])
//                    //                {
//                    //                    combinationRow = -1;
//                    //                }
//                    //            }
...
//                    //        bool match = true;
//                    //        for (int i = 0; i < combinationItems.Length - 1; i++)
//                    //        {
//                    //            int j = i + 1;
//                    //            if (predictedCollections[combinationItems[i] * PredictedCollectionsMaxSize + predictionPtrs[i]] != predictedCollections[combinationItems[j] * PredictedCollectionsMaxSize + predictionPtrs[j]])
//                    //            {
//                    //                match = false;
//                    //                break;
//                    //            }
...
//                    //        if (match)
//                    //        {
//                    //            int matchRow = predictedCollections[combinationItems[0] * PredictedCollectionsMaxSize + predictionPtrs[0]];
//                    //            predictionPtrs[0]++;

//                    //            combinationRow = matchRow;
//                    //        }
...
//                    //        if (((DataItem)dataItem == DataItem.OpenChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Open, DataSet.DataColumns.Open, false) > DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.CloseChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Close, DataSet.DataColumns.Close, false) > DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.VolumeChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Volume, DataSet.DataColumns.Volume, false) > DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.CloseOpenDif
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Close, DataSet.DataColumns.Open, true) > DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.OpenPrevCloseDif
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Open, DataSet.DataColumns.Close, false) > DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.NegativeOpenChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Open, DataSet.DataColumns.Open, false) < -DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.NegativeCloseChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Close, DataSet.DataColumns.Close, false) < -DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.NegativeVolumeChange
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Volume, DataSet.DataColumns.Volume, false) < -DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.NegativeCloseOpenDif
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Close, DataSet.DataColumns.Open, true) < -DataAnalyzer.PredictionErrorRange)
//                    //            || ((DataItem)dataItem == DataItem.NegativeOpenPrevCloseDif
//                    //            && CalculateChange(dataSet, combinationRow, range, DataSet.DataColumns.Open, DataSet.DataColumns.Close, false) < -DataAnalyzer.PredictionErrorRange))
//                    //        {
//                    //            correctPredictions[analyzeCombinationNum]++;
//                    //        }
...
//        private void DoAnalyzerCombination(int resultsStart, int combinationItem,
//                                           deviceptr<double> dataSet,
//                                           deviceptr<int> predictedCollections,
//                                           deviceptr<int> predictedCollectionsSizes,
//                                           deviceptr<int> analyzeCombinationsDataItems,
//                                           deviceptr<int> analyzeCombinationsRanges,
...
//        private void DoAnalyzerCombination(int resultsStart, int[] combinationItems, int[] predictionPtrs, int combinationSize,
//                                           deviceptr<double> dataSet,
//                                           deviceptr<ulong> combinations,
//                                           deviceptr<int> predictedCollections,
//                                           deviceptr<int> predictedCollectionsSizes,
//                                           deviceptr<ulong> analyzeCombinationsItems,
//                                           deviceptr<double> analyzeResults)
//        {
//            int combinationRow = 0;
//            int numOfCombinations = 0;
...
//            while ((combinationRow = GetNextCombinationRow(combinationItems, predictionPtrs, combinationSize, predictedCollections, predictedCollectionsSizes)) != -1)
//            {
//                for (int analyzeCombinationNum = 0; analyzeCombinationNum < m_AnalyzesNum; analyzeCombinationNum++)
//                {
//                    ulong analyzeCombinationItem = analyzeCombinationsItems[analyzeCombinationNum];
//                    int rangeNum = GetRangeNum(analyzeCombinationItem);
//                    int dataItem = (int)(analyzeCombinationItem >> (rangeNum * 10));

//                    if (IsContainsPrediction(dataSet, dataItem, m_AnalyzeRanges[rangeNum], combinationRow, DataAnalyzer.PredictionErrorRange, -DataAnalyzer.PredictionErrorRange))
//                    {
//                        correctPredictions[analyzeCombinationNum]++;
//                    }
//                }
...
//        private int GetNextCombinationRow(int[] combinationItems, int[] predictionPtrs, int combinationSize, deviceptr<int> predictedCollections, deviceptr<int> predictedCollectionsSizes)
//        {
//            while (predictionPtrs[0] < predictedCollectionsSizes[combinationItems[0]])
//            {
//                for (int i = 0; i < combinationSize - 1; i++)
